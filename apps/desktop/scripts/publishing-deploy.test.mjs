...
async function fixture(mode = "success") {
  ...
  return { options, calls, remote, root, dependencies: { run, now: () => clock, sleep: async ms => { clock += ms } } }
}

test("missing or changed workflow blocks before snapshot upload", async () => {
  for (const mode of ["missing", "wrong-workflow", "wrong-runner"]) {
